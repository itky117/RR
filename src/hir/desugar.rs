...
impl Desugarer {
    ...
    fn desugar_item(&mut self, item: HirItem) -> RR<HirItem> {
        match item {
            ...
            HirItem::Stmt(s) => {
                Ok(HirItem::Stmt(self.desugar_stmt(s)?))
            }
            _ => Ok(item),
        }
    }
    ...
    fn desugar_stmt(&mut self, stmt: HirStmt) -> RR<HirStmt> {
        match stmt {
            HirStmt::Let { local, name, ty, init, span } => {
                let init = if let Some(e) = init { Some(self.desugar_expr(e)?) } else { None };
                Ok(HirStmt::Let { local, name, ty, init, span })
            }
            HirStmt::Assign { target, value, span } => {
                Ok(HirStmt::Assign { target: self.desugar_lvalue(target)?, value: self.desugar_expr(value)?, span })
            }
            HirStmt::If { cond, then_blk, else_blk, span } => {
                 Ok(HirStmt::If { 
                    cond: self.desugar_expr(cond)?, 
                    then_blk: self.desugar_block(then_blk)?,
                    else_blk: if let Some(e) = else_blk { Some(self.desugar_block(e)?) } else { None },
                    span 
                 })
            }
            ...
            HirStmt::Expr { expr, span } => {
                Ok(HirStmt::Expr { expr: self.desugar_expr(expr)?, span })
            }
            HirStmt::For { iter, body, span } => {
                let iter = match iter {
                    HirForIter::Range { var, start, end, inclusive } => HirForIter::Range { var, start: self.desugar_expr(start)?, end: self.desugar_expr(end)?, inclusive },
                    HirForIter::SeqLen { var, len } => HirForIter::SeqLen { var, len: self.desugar_expr(len)? },
                    HirForIter::SeqAlong { var, xs } => HirForIter::SeqAlong { var, xs: self.desugar_expr(xs)? },
                };
                Ok(HirStmt::For { iter, body: self.desugar_block(body)?, span })
            }
            HirStmt::Return { value, span } => {
                let v = if let Some(e) = value { Some(self.desugar_expr(e)?) } else { None };
    ...
    fn desugar_lvalue(&mut self, lv: HirLValue) -> RR<HirLValue> {
        match lv {
            HirLValue::Index { base, index } => {
                ...
                Ok(HirLValue::Index { base: self.desugar_expr(base)?, index: new_index })
            }
            HirLValue::Field { base, name } => Ok(HirLValue::Field { base: self.desugar_expr(base)?, name }),
            _ => Ok(lv),
        }
    }
    ...
    fn desugar_expr(&mut self, expr: HirExpr) -> RR<HirExpr> {
        match expr {
            HirExpr::Match { scrut, arms } => {
                ...
                Ok(HirExpr::Match { scrut, arms: new_arms })
            }
            HirExpr::Binary { op, lhs, rhs } => Ok(HirExpr::Binary { op, lhs: Box::new(self.desugar_expr(*lhs)?), rhs: Box::new(self.desugar_expr(*rhs)?) }),
            HirExpr::Range { start, end } => Ok(HirExpr::Range { start: Box::new(self.desugar_expr(*start)?), end: Box::new(self.desugar_expr(*end)?) }),
            HirExpr::Index { base, index } => {
                let mut new_index = Vec::with_capacity(index.len());
                for idx in index {
                    new_index.push(self.desugar_expr(idx)?);
                }
                Ok(HirExpr::Index { base: Box::new(self.desugar_expr(*base)?), index: new_index })
            }
            HirExpr::Call(mut c) => {
            ...
            HirExpr::ListLit(items) => {
                 ...
                 for (n, e) in items { new_items.push((n, self.desugar_expr(e)?)); }
    ...
    fn desugar_match(&mut self, scrut: HirExpr, arms: Vec<HirMatchArm>) -> RR<HirExpr> {
        let span = Span { start_byte: 0, end_byte: 0, start_line: 0, end_line: 0, start_col: 0, end_col: 0 };
        ...
        for arm in arms.into_iter().rev() {
            ...
            let cond = if let Some(g) = arm.guard {
                HirExpr::Binary { op: HirBinOp::And, lhs: Box::new(pat_check), rhs: Box::new(g) }
            } else {
                pat_check
            };
            ...
            let branch_expr = if bind_stmts.is_empty() {
                ...
                bind_stmts.push(HirStmt::Expr { expr: desugared_body, span });
                HirExpr::Block(HirBlock { stmts: bind_stmts, span })
            };
        ...
        Ok(HirExpr::Block(HirBlock {
            stmts: vec![
                let_stmt,
                HirStmt::Expr { expr: current_expr, span }
            ],
            span
        }))
    }
    ...
    fn pattern_bindings(&mut self, root: LocalId, pat: &HirPat, stmts: &mut Vec<HirStmt>, span: Span) -> RR<()> {
        match pat {
