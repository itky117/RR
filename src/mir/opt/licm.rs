...
use crate::mir::*;
use crate::mir::analyze::{effects, alias};
...
impl MirLicm {
    ...
    fn hoist_invariants(&self, fn_ir: &mut FnIR, loop_info: &crate::mir::opt::loop_analysis::LoopInfo) -> bool {
        let mut changed = false;
        ...
        for (vid, bb) in &val_to_bb {
            if !loop_info.body.contains(bb) { continue; }
        ...
        for &bb in &loop_info.body {
            for instr in &fn_ir.blocks[bb].instrs {
                match instr {
                    Instr::Assign { src, .. } | Instr::Eval { val: src, .. } | Instr::StoreIndex1D { val: src, .. } | Instr::StoreIndex2D { val: src, .. } => {
                        // Already hoisted?
                        if let Some(name) = &fn_ir.values[*src].origin_var {
                            if name.starts_with("licm_") { continue; }
                        }

                        if self.is_hoistable(fn_ir, *src, &invariants, loop_info, &val_to_bb, &loop_ctx) {
                            // Deduplicate
        ...
        if hoist_candidates.is_empty() { return false; }
        ...
        let pre_headers: Vec<BlockId> = header_preds.into_iter()
    ...
    fn compute_invariants(&self, fn_ir: &FnIR, loop_info: &crate::mir::opt::loop_analysis::LoopInfo, val_to_bb: &std::collections::HashMap<ValueId, BlockId>, loop_ctx: &LoopEffectCtx) -> std::collections::HashSet<ValueId> {
        ...
        while changed {
            ...
            for (id, _val) in fn_ir.values.iter().enumerate() {
                let val_id = id;
                if invariants.contains(&val_id) { continue; }
    ...
    fn is_hoistable(&self, fn_ir: &FnIR, val_id: ValueId, invariants: &std::collections::HashSet<ValueId>, loop_info: &crate::mir::opt::loop_analysis::LoopInfo, val_to_bb: &std::collections::HashMap<ValueId, BlockId>, loop_ctx: &LoopEffectCtx) -> bool {
        ...
        self.is_value_invariant(fn_ir, val_id, invariants, loop_ctx) && !matches!(fn_ir.values[val_id].kind, ValueKind::Phi { .. })
    }
    
    fn is_value_invariant(&self, fn_ir: &FnIR, val_id: ValueId, invariants: &std::collections::HashSet<ValueId>, loop_ctx: &LoopEffectCtx) -> bool {
        let val = &fn_ir.values[val_id];
        match &val.kind {
            ValueKind::Const(_) | ValueKind::Param { .. } => true,
            ValueKind::Binary { lhs, rhs, .. } => invariants.contains(lhs) && invariants.contains(rhs),
            ValueKind::Unary { rhs, .. } => invariants.contains(rhs),
            ValueKind::Range { start, end } => invariants.contains(start) && invariants.contains(end),
            ...
            ValueKind::Phi { args } => {
                ...
                args.iter().all(|(a, _)| *a == val_id || invariants.contains(a))
            }
            // Be conservative
