    ...
    match kind {
        ValueKind::Binary { op, lhs, rhs } => {
            ...
            match op {
                BinOp::Add => {
                    // x + 0 -> x
                    if is_const_zero(r_kind) { return Some(fn_ir.values[*lhs].kind.clone()); }
                    // 0 + x -> x
                    if is_const_zero(l_kind) { return Some(fn_ir.values[*rhs].kind.clone()); }
                }
                BinOp::Sub => {
                    // x - 0 -> x
                    if is_const_zero(r_kind) { return Some(fn_ir.values[*lhs].kind.clone()); }
                    ...
                    if lhs == rhs { return Some(ValueKind::Const(Lit::Int(0))); }
                }
                BinOp::Mul => {
                    // x * 1 -> x
                    if is_const_one(r_kind) { return Some(fn_ir.values[*lhs].kind.clone()); }
                    // 1 * x -> x
                    if is_const_one(l_kind) { return Some(fn_ir.values[*rhs].kind.clone()); }
                    // x * 0 -> 0
                    if is_const_zero(r_kind) { return Some(ValueKind::Const(Lit::Int(0))); }
                    // 0 * x -> 0
                    if is_const_zero(l_kind) { return Some(ValueKind::Const(Lit::Int(0))); }
                    // x * 2 -> x + x
                    if is_const_two(r_kind) {
                        return Some(ValueKind::Binary { op: BinOp::Add, lhs: *lhs, rhs: *lhs });
                    }
                }
                BinOp::Div => {
                    // x / 1 -> x
        ...
        ValueKind::Unary { op, rhs } => {
            ...
            match op {
                UnaryOp::Not => {
                    // !!x -> x
                    if let ValueKind::Unary { op: UnaryOp::Not, rhs: inner_rhs } = r_kind {
...
fn is_const_zero(kind: &ValueKind) -> bool {
    matches!(kind, ValueKind::Const(Lit::Int(0)) | ValueKind::Const(Lit::Float(0.0)))
}

fn is_const_one(kind: &ValueKind) -> bool {
    matches!(kind, ValueKind::Const(Lit::Int(1)) | ValueKind::Const(Lit::Float(1.0)))
}

fn is_const_two(kind: &ValueKind) -> bool {
    matches!(kind, ValueKind::Const(Lit::Int(2)) | ValueKind::Const(Lit::Float(2.0)))
}
