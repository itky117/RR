...
pub fn emit_parallel_copy(fn_ir: &mut FnIR, out_instrs: &mut Vec<Instr>, moves: Vec<Move>, span: Span) {
    ...
    while !pending.is_empty() {
        ...
        if let Some(idx) = candidate_idx {
            ...
            out_instrs.push(Instr::Assign { dst: m.dst, src: m.src, span });
        } else {
            ...
            let save_val_id = fn_ir.add_value(ValueKind::Load { var: victim_var.clone() }, span, Facts::empty(), None);
            out_instrs.push(Instr::Assign { dst: temp_var.clone(), src: save_val_id, span });
...
fn value_reads_var(fn_ir: &FnIR, src: ValueId, var: &VarId) -> bool {
    ...
    match &val.kind {
        ...
        ValueKind::Binary { lhs, rhs, .. } => value_reads_var(fn_ir, *lhs, var) || value_reads_var(fn_ir, *rhs, var),
        ValueKind::Unary { rhs, .. } => value_reads_var(fn_ir, *rhs, var),
        ValueKind::Call { args, .. } => args.iter().any(|a| value_reads_var(fn_ir, *a, var)),
        ValueKind::Phi { .. } => false,
        ValueKind::Index1D { base, idx, .. } => value_reads_var(fn_ir, *base, var) || value_reads_var(fn_ir, *idx, var),
...
fn replace_var_read(fn_ir: &mut FnIR, src: ValueId, old_var: &VarId, new_var: &VarId) -> ValueId {
    ...
    let new_kind = match val.kind {
        ValueKind::Load { var } => {
            if &var == old_var {
                ValueKind::Load { var: new_var.clone() }
            } else {
                return src;
            }
        },
        ...
        ValueKind::Index1D { base, idx, is_safe, is_na_safe } => {
            ...
            ValueKind::Index1D { base: b, idx: i, is_safe, is_na_safe }
        },
        ValueKind::Index2D { base, r, c } => {
            let b = replace_var_read(fn_ir, base, old_var, new_var);
