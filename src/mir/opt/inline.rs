...
impl MirInliner {
    ...
    fn inline_calls(&self, caller: &mut FnIR, all_fns: &HashMap<String, FnIR>) -> bool {
        ...
        'scan: for bid in 0..caller.blocks.len() {
            for (idx, instr) in caller.blocks[bid].instrs.iter().enumerate() {
                 if let Some((callee_name, args, target_val, call_dst, call_span)) = self.analyze_instr(caller, instr, all_fns) {
                     if callee_name == caller.name { continue; }
                     ...
                     if let Some(callee) = all_fns.get(&callee_name) {
                         ...
                         if self.should_inline(callee, caller) {
                             candidate = Some((bid, idx, callee_name, args, target_val, call_dst, call_span));
                             break 'scan;
                         }
                     }
                 }
        ...
        if let Some((bid, idx, callee_name, args, target_val, call_dst, call_span)) = candidate {
            ...
            self.perform_inline(caller, bid, idx, &args, target_val, call_dst, callee, call_span);
            changed = true;
        }
    ...
    fn analyze_instr(&self, caller: &FnIR, instr: &Instr, all_fns: &HashMap<String, FnIR>) -> Option<(String, Vec<ValueId>, ValueId, Option<VarId>, Span)> {
        match instr {
            Instr::Assign { dst, src, span, .. } => {
                if let ValueKind::Call { callee, args, .. } = &caller.values[*src].kind {
                    ...
                    if all_fns.contains_key(base_name) {
                        return Some((base_name.to_string(), args.clone(), *src, Some(dst.clone()), *span));
                    }
                }
            }
    ...
    fn perform_inline(&self, caller: &mut FnIR, call_block: BlockId, instr_idx: usize, call_args: &[ValueId], call_val_target: ValueId, call_dst: Option<VarId>, callee: &FnIR, call_span: Span) {
        ...
        for (cvid, val) in callee.values.iter().enumerate() {
            if let ValueKind::Param { index } = val.kind {
                if index < call_args.len() {
                    if mutated_params.contains(&index) {
                        ...
                        let load_id = caller.add_value(ValueKind::Load { var: mapped_var.clone() }, call_span, Facts::empty(), None);
                    ...
                    let dummy = caller.add_value(ValueKind::Const(crate::syntax::ast::Lit::Null), call_span, Facts::empty(), None);
            ...
            let new_vid = caller.add_value(ValueKind::Const(crate::syntax::ast::Lit::Null), val.span, val.facts.clone(), None);
        ...
        for (cvid, val) in callee.values.iter().enumerate() {
            if let ValueKind::Param { .. } = val.kind { continue; }
        ...
        let post_split: Vec<Instr> = caller.blocks[call_block].instrs.drain((instr_idx + 1)..).collect();
            ...
            if let Terminator::Return(ret_opt) = &caller.blocks[nbid].term {
                if let Some(ret_val) = ret_opt {
                    returns.push((*ret_val, nbid));
                     ...
                     let null_val = caller.add_value(ValueKind::Const(crate::syntax::ast::Lit::Null), call_span, Facts::empty(), None);
        ...
        let res_id: ValueId = if returns.is_empty() {
            ...
            caller.blocks[continuation_bb].instrs.insert(0, Instr::Eval { val: call_val_target, span: call_span });
        ...
        if let Some(dst) = call_dst {
            caller.blocks[continuation_bb].instrs.insert(0, Instr::Assign { dst, src: res_id, span: call_span });
        }
    ...
    fn inline_value_calls(&self, caller: &mut FnIR, all_fns: &HashMap<String, FnIR>) -> bool {
        for val_id in 0..caller.values.len() {
            let (callee_name, args) = match &caller.values[val_id].kind {
                ValueKind::Call { callee, args, .. } => (self.resolve_callee_name(callee).to_string(), args.clone()),
                _ => continue,
            };
            ...
            if let Some(replacement) = self.inline_call_value(caller, val_id, callee, ret_val, &args) {
    ...
    fn can_inline_expr(&self, callee: &FnIR) -> Option<ValueId> {
        ...
        for bid in &reachable {
            ...
            for instr in &blk.instrs {
                match instr {
                    Instr::StoreIndex1D { .. } | Instr::StoreIndex2D { .. } | Instr::Eval { .. } => return None,
                    _ => {}
                }
            }
            match blk.term {
    ...
    fn reachable_blocks(&self, callee: &FnIR) -> HashSet<BlockId> {
        ...
        while let Some(bid) = queue.pop_front() {
            ...
            match blk.term {
                Terminator::Goto(t) => {
                    if reachable.insert(t) { queue.push_back(t); }
                }
                Terminator::If { then_bb, else_bb, .. } => {
                    if reachable.insert(then_bb) { queue.push_back(then_bb); }
                    if reachable.insert(else_bb) { queue.push_back(else_bb); }
                }
                _ => {}
            }
        }
        ...
        let clone_value = |vid: ValueId,
            ...
            fn clone_rec(
                ...
                match &val.kind {
                    ...
                    ValueKind::Const(lit) => {
                        let new_id = caller.add_value(ValueKind::Const(lit.clone()), val.span, val.facts.clone(), None);
                    ...
                    ValueKind::Binary { op, lhs, rhs } => {
                        ...
                        let new_id = caller.add_value(ValueKind::Binary { op: *op, lhs: l, rhs: r }, val.span, val.facts.clone(), None);
                        map.insert(vid, new_id);
                        return Some(new_id);
                    }
                    ValueKind::Unary { op, rhs } => {
                        let r = clone_rec(*rhs, caller, callee, map, args)?;
                        let new_id = caller.add_value(ValueKind::Unary { op: *op, rhs: r }, val.span, val.facts.clone(), None);
                        map.insert(vid, new_id);
                        return Some(new_id);
                    }
                    ValueKind::Len { base } => {
                        let b = clone_rec(*base, caller, callee, map, args)?;
                        let new_id = caller.add_value(ValueKind::Len { base: b }, val.span, val.facts.clone(), None);
                        map.insert(vid, new_id);
                        return Some(new_id);
                    }
                    ValueKind::Indices { base } => {
                        let b = clone_rec(*base, caller, callee, map, args)?;
                        let new_id = caller.add_value(ValueKind::Indices { base: b }, val.span, val.facts.clone(), None);
                        map.insert(vid, new_id);
                        return Some(new_id);
                    }
                    ValueKind::Range { start, end } => {
                        ...
                        let new_id = caller.add_value(ValueKind::Range { start: s, end: e }, val.span, val.facts.clone(), None);
                        map.insert(vid, new_id);
                        return Some(new_id);
                    }
                    ValueKind::Index1D { base, idx, is_safe, is_na_safe } => {
                        ...
                        let new_id = caller.add_value(ValueKind::Index1D { base: b, idx: i, is_safe: *is_safe, is_na_safe: *is_na_safe }, val.span, val.facts.clone(), None);
                        map.insert(vid, new_id);
                        return Some(new_id);
                    }
                    ValueKind::Index2D { base, r, c } => {
                        ...
                        let new_id = caller.add_value(ValueKind::Index2D { base: b, r: rv, c: cv }, val.span, val.facts.clone(), None);
                        map.insert(vid, new_id);
                        return Some(new_id);
                    }
                    _ => None,
    ...
    fn remap_value_kind(&self, kind: &mut ValueKind, map: &mut InlineMap) {
        match kind {
            ValueKind::Binary { lhs, rhs, .. } => {
                if let Some(&n) = map.v.get(lhs) { *lhs = n; }
                if let Some(&n) = map.v.get(rhs) { *rhs = n; }
            }
            ValueKind::Unary { rhs, .. } => {
                if let Some(&n) = map.v.get(rhs) { *rhs = n; }
            }
            ValueKind::Call { args, .. } => {
                for a in args { if let Some(&n) = map.v.get(a) { *a = n; } }
            }
            ValueKind::Phi { args } => {
                for (v, b) in args { 
                    if let Some(&n) = map.v.get(v) { *v = n; }
                    if let Some(&n) = map.b.get(b) { *b = n; }
                }
            }
            ValueKind::Index1D { base, idx, .. } => {
                if let Some(&n) = map.v.get(base) { *base = n; }
                if let Some(&n) = map.v.get(idx) { *idx = n; }
            }
            ValueKind::Index2D { base, r, c } => {
                if let Some(&n) = map.v.get(base) { *base = n; }
                if let Some(&n) = map.v.get(r) { *r = n; }
                if let Some(&n) = map.v.get(c) { *c = n; }
            }
             ValueKind::Len { base } | ValueKind::Indices { base } => {
                if let Some(&n) = map.v.get(base) { *base = n; }
             }
             ValueKind::Range { start, end } => {
                if let Some(&n) = map.v.get(start) { *start = n; }
                if let Some(&n) = map.v.get(end) { *end = n; }
             }
            ValueKind::Load { var } => {
    ...
    fn remap_instr(&self, instr: &mut Instr, map: &mut InlineMap) {
        match instr {
            Instr::Assign { dst, src, .. } => {
                if let Some(&n) = map.v.get(src) { *src = n; }
                let mapped = map.map_var(dst);
                *dst = mapped;
            }
            Instr::Eval { val, .. } => {
                if let Some(&n) = map.v.get(val) { *val = n; }
            }
    ...
    fn remap_term(&self, term: &mut Terminator, map: &InlineMap) {
        match term {
            ...
            Terminator::If { cond, then_bb, else_bb } => {
                if let Some(&n) = map.v.get(cond) { *cond = n; }
                if let Some(&n) = map.b.get(then_bb) { *then_bb = n; }
                if let Some(&n) = map.b.get(else_bb) { *else_bb = n; }
            }
            Terminator::Return(Some(v)) => {
                if let Some(&n) = map.v.get(v) { *v = n; }
            }
    ...
    fn replace_uses(&self, fn_ir: &mut FnIR, old: ValueId, new: ValueId) {
        ...
        for val in &mut fn_ir.values {
            match &mut val.kind {
                ...
                ValueKind::Call { args, .. } => {
                    for a in args { if *a == old { *a = new; } }
                }
                ValueKind::Phi { args } => {
                    for (v, _) in args { if *v == old { *v = new; } }
                }
                ValueKind::Index1D { base, idx, .. } => {
        ...
        for blk in &mut fn_ir.blocks {
            for instr in &mut blk.instrs {
                match instr {
                    Instr::Assign { src, .. } => { if *src == old { *src = new; } },
                    Instr::Eval { val, .. } => { if *val == old { *val = new; } },
                    Instr::StoreIndex1D { base, idx, val, .. } => {
                        if *base == old { *base = new; }
                        if *idx == old { *idx = new; }
                        if *val == old { *val = new; }
            ...
            match &mut blk.term {
                Terminator::If { cond, .. } => { if *cond == old { *cond = new; } },
