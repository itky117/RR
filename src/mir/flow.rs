...
impl Interval {
    ...
    pub const TOP: Interval = Interval { min: i64::MIN, max: i64::MAX };
    pub const BOTTOM: Interval = Interval { min: i64::MAX, max: i64::MIN }; // Empty set representation
    ...
    pub fn join(&self, other: &Interval) -> Interval {
        if *self == Self::BOTTOM { return *other; }
        if *other == Self::BOTTOM { return *self; }
    ...
    pub fn add(&self, other: &Interval) -> Interval {
        if *self == Self::BOTTOM || *other == Self::BOTTOM { return Self::BOTTOM; }
...
impl Facts {
    ...
    pub fn empty() -> Self {
        Self { flags: 0, interval: Interval::BOTTOM } // Start at Bottom for analysis
    }
    ...
    pub fn join(&self, other: &Self) -> Self {
        ...
        if f.interval.min >= 0 { f.add(Self::NON_NEG); }
        f
    }
}
...
impl DataflowSolver {
    ...
    fn transfer(val: &Value, facts: &HashMap<ValueId, Facts>, _fn_ir: &FnIR) -> Facts {
        ...
        match &val.kind {
            ...
            ValueKind::Phi { args } => {
                // Join all inputs.
                if args.is_empty() { return Facts::empty(); }
            ...
            ValueKind::Binary { op, lhs, rhs } => {
                ...
                // Re-derive
                if res.interval.min >= 0 { res.add(Facts::NON_NEG); }
                
                res
            },
            
            ValueKind::Unary { op: crate::syntax::ast::UnaryOp::Neg, rhs } => {
                let rf = facts.get(rhs).cloned().unwrap_or(Facts::empty());
                let mut res = Facts::empty();
                if rf.has(Facts::IS_VECTOR) { res.add(Facts::IS_VECTOR); }
